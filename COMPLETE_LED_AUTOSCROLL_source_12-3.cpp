// Psychedelic Clock for ESP32-2432S028 Display with LED and Auto-Scroll
...
// LED pin (back of ESP32-2432S028)
#define LED_PIN 4  // GPIO4 controls the onboard LED
...
const unsigned long autoScrollInterval = 60000; // 1 minute per effect

// LED control variables
bool ledEnabled = true;
unsigned long lastLedUpdate = 0;
const unsigned long ledUpdateInterval = 50; // Update LED every 50ms
...
// Button press tracking for auto-scroll toggle
unsigned long buttonPressStart = 0;
bool buttonPressed = false;
const unsigned long holdTime = 2000; // Hold for 2 seconds to toggle auto-scroll
...
void initLED() {
  ...
  Serial.println("LED initialized on GPIO4");
}

void updateLED() {
  if (!ledEnabled || millis() - lastLedUpdate < ledUpdateInterval) {
    return;
  }
  
  lastLedUpdate = millis();
  
  // Create rainbow effect based on animation mode and time
  ledHue = (currentMode * 6 + millis() / 100) % 360;
  
  // Convert HSV to RGB for LED control
  float h = ledHue / 60.0;
  float s = 1.0;
  float v = ledBrightness / 255.0;
  ...
  int pwmValue = (int)(255 * ((r + g + b) / 3.0));
  analogWrite(LED_PIN, pwmValue);
}

void handleAutoScroll() {
  if (autoScrollEnabled && millis() - lastAutoScroll >= autoScrollInterval) {
    currentMode = (AnimationMode)((currentMode + 1) % MODE_COUNT);
    animationTime = 0; // Reset animation
    lastAutoScroll = millis();
    Serial.printf("Auto-scroll: Switched to mode %d\n", currentMode);
  }
}
...
void setup() {
  ...
  gfx->invertDisplay(false);     // For newer hardware batch (2024/2025)
  Serial.println("Display inversion disabled - for newer hardware batch");
  ...
  fillScreenExceptClock(RGB565_BLACK);
  lastAutoScroll = millis(); // Initialize auto-scroll timer
  Serial.println("Psychedelic Clock ready with LED and Auto-scroll!");
  Serial.println("Hold BOOT button for 2 seconds to toggle auto-scroll");
}
...
void loop() {
  ...
  // Handle button press for auto-scroll toggle (hold for 2 seconds)
  if (digitalRead(BOOT_PIN) == LOW) {
    if (!buttonPressed) {
      buttonPressed = true;
      buttonPressStart = millis();
    } else if (millis() - buttonPressStart >= holdTime) {
      // Toggle auto-scroll after holding for 2 seconds
      autoScrollEnabled = !autoScrollEnabled;
      Serial.printf("Auto-scroll %s\n", autoScrollEnabled ? "ENABLED" : "DISABLED");
      if (autoScrollEnabled) {
        lastAutoScroll = millis(); // Reset timer when enabling
      }
      buttonPressed = false; // Prevent repeated toggles
      delay(500); // Debounce
    }
  } else {
    if (buttonPressed && millis() - buttonPressStart < holdTime) {
      // Short press - manual mode change
      skipRequested = true;
    }
    buttonPressed = false;
  }
  
  // Handle manual mode switching (short button press)
  if (skipRequested) {
    uint32_t now = millis();
    if (now - lastPress >= BOOT_BUTTON_DEBOUNCE_TIME) {
      ...
      // Reset auto-scroll timer when manually changing modes
      if (autoScrollEnabled) {
        lastAutoScroll = millis();
      }
    }
  ...
  // Update LED based on current animation
  updateLED();
...
void drawClockOverlay() {
  ...
  // Effect number and auto-scroll indicator
  char statusStr[16];
  sprintf(statusStr, "#%d%s", (int)currentMode, autoScrollEnabled ? " A" : "");
  int statusX = gfx->width() - 70;
  gfx->fillRect(statusX, overlayClockY, 60, 16, RGB565_BLACK);
  gfx->setCursor(statusX, overlayClockY);
  gfx->print(statusStr);
...
void handleRoot() {
  ...
  html += "<p><strong>Auto-scroll:</strong> " + String(autoScrollEnabled ? "ON (1min/effect)" : "OFF") + "</p>";
  html += "<p><strong>LED:</strong> " + String(ledEnabled ? "ON" : "OFF") + "</p>";
  ...
  html += "<p><strong>Current Effect:</strong> #" + String((int)currentMode) + "</p>";
  ...
  html += "<a href='/led'><button>Toggle LED</button></a><br><br>";
...
void handleToggleAutoScroll() {
  ...
  server.send(200, "text/html", "<html><body><h1>Auto-scroll " + String(autoScrollEnabled ? "Enabled" : "Disabled") + "!</h1><a href='/'>Back</a></body></html>");
}
...
void handleToggleLED() {
  ...
  server.send(200, "text/html", "<html><body><h1>LED " + String(ledEnabled ? "Enabled" : "Disabled") + "!</h1><a href='/'>Back</a></body></html>");
}
