...
enum AnimationMode {
  ...
  JULIA_STANDALONE = 58,
  RETRO_GEOMETRY = 59,
  // BATCH 1: XScreensaver Effects (5 effects)
  ABSTRACTILE = 60,
  ANALOGTV = 61,
  ANEMONE = 62, 
  APOLLONIAN = 63,
  BARCODE = 64,
  // BATCH 2: XScreensaver Effects (7 effects)
  BLASTER = 65,
  BLITSPIN = 66,
  BOUBOULE = 67,
  BOXFIT = 68,
  ...
  BUBBLES = 70,
  BUMPS = 71,
  // BATCH 3: XScreensaver Effects (7 effects fit in memory)
  COMPASS = 72,
  CORAL = 73,
  CRITICAL = 74,
  CRYSTAL = 75,
...
// Plasma field variables (minimal size)
uint8_t plasmaBuffer[80 * 120]; // Quarter resolution buffer
...
void drawApollonian();
void drawBarcode();
// BATCH 2: XScreensaver Effects (10 more)
void drawBlaster();
void drawBlitSpin();
void drawBouboule();
void drawBoxFit();
...
void drawBubbles();
void drawBumps();
// BATCH 3: XScreensaver Effects (10 more)
void drawCompass();
void drawCoral();
void drawCritical();
void drawCrystal();
...
void setup() {
  ...
  server.on("/autoscroll", handleToggleAutoScroll);
  server.on("/setautoscrolltime", handleSetAutoScrollTime);
  server.begin();
...
void loop() {
  ...
  // Handle boot button press (interrupt-based)
  if (skipRequested) {
    uint32_t now = millis();
    if (now - lastPress >= BOOT_BUTTON_DEBOUNCE_TIME) {
      ...
      Serial.printf("Button press: Switched to mode %d\n", currentMode);
      lastPress = now;
    }
    skipRequested = false;
  }
  
  // Check for long press to toggle auto-scroll
  static bool longPressChecked = false;
  ...
  if (autoScrollEnabled) {
    ...
    if (now - lastAutoScrollTime >= autoScrollInterval) {
      currentMode = (AnimationMode)((currentMode + 1) % MODE_COUNT);
      animationTime = 0; // Reset animation
      lastAutoScrollTime = now;
      Serial.printf("Auto-scrolled to mode %d\n", currentMode);
    }
  }
...
void handleRoot() {
  ...
  html += "<h3>Auto-Scroll</h3>";
  html += "<p><strong>Status:</strong> " + String(autoScrollEnabled ? "ENABLED" : "DISABLED") + "</p>";
  html += "<p><strong>Interval:</strong> " + String(autoScrollInterval/1000) + " seconds</p>";
  html += "<a href='/autoscroll'><button>Toggle Auto-Scroll</button></a><br><br>";
  html += "<form action='/setautoscrolltime' method='get'>";
  html += "Auto-scroll seconds: <input type='number' name='seconds' min='5' max='300' value='" + String(autoScrollInterval/1000) + "'><br><br>";
  html += "<input type='submit' value='Set Auto-Scroll Time' style='padding:10px;font-size:16px;'>";
  html += "</form>";
  html += "<p><small>Hold BOOT button for 2+ seconds to toggle auto-scroll</small></p>";
...
void handleToggleAutoScroll() {
  autoScrollEnabled = !autoScrollEnabled;
  lastAutoScrollTime = millis(); // Reset timer
  String message = "<html><body><h1>Auto-scroll " + String(autoScrollEnabled ? "ENABLED" : "DISABLED") + "!</h1>";
  message += "<p>Current interval: " + String(autoScrollInterval/1000) + " seconds</p>";
  message += "<a href='/'>Back</a></body></html>";
...
void handleSetAutoScrollTime() {
  if (server.hasArg("seconds")) {
    ...
    if (seconds >= 5 && seconds <= 300) { // 5 seconds to 5 minutes
      autoScrollInterval = seconds * 1000;
      lastAutoScrollTime = millis(); // Reset timer
      String message = "<html><body><h1>Auto-scroll time set to " + String(seconds) + " seconds!</h1>";
      message += "<a href='/'>Back</a></body></html>";
      server.send(200, "text/html", message);
    } else {
      server.send(400, "text/html", "<html><body><h1>Invalid time! Use 5-300 seconds.</h1><a href='/'>Back</a></body></html>");
    }
  } else {
    server.send(400, "text/html", "<html><body><h1>Missing seconds parameter!</h1><a href='/'>Back</a></body></html>");
  }
}

// ========== BATCH 1: XSCREENSAVER EFFECTS ==========

void drawAbstractile() {
  // Abstract tile patterns - geometric shapes
  static float tileTime = 0;
  tileTime += 0.02f;
  
  int tileSize = 20 + sin(tileTime) * 10;
  for (int y = 0; y < getAnimationHeight(); y += tileSize) {
    for (int x = 0; x < getAnimationWidth(); x += tileSize) {
      float phase = (x + y) * 0.01f + tileTime;
      uint8_t r = (sin(phase) + 1) * 127;
      uint8_t g = (sin(phase + 2) + 1) * 127;
      uint8_t b = (sin(phase + 4) + 1) * 127;
      uint16_t color = gfx->color565(r, g, b);
...
void drawAnalogTV() {
  ...
  for (int i = 0; i < 1000; i++) {
    int x = random(getAnimationWidth());
    int y = random(getAnimationHeight());
    uint8_t intensity = random(256);
    uint16_t color = gfx->color565(intensity, intensity, intensity);
    gfx->drawPixel(x, y, color);
  }
  ...
  // Add scan lines
  if (frameNum % 3 == 0) {
    int scanY = random(getAnimationHeight());
    gfx->drawLine(0, scanY, getAnimationWidth(), scanY, RGB565_WHITE);
  }
}

void drawAnemone() {
  // Sea anemone tentacles
  static float tentacleTime = 0;
  tentacleTime += 0.05f;
  
  int centerX = getAnimationWidth() / 2;
  int centerY = getAnimationHeight() / 2;
  int numTentacles = 12;
  
  for (int i = 0; i < numTentacles; i++) {
    float angle = (i * 2.0 * PI / numTentacles) + tentacleTime * 0.5;
    for (int r = 10; r < 80; r += 5) {
      float waveOffset = sin(tentacleTime + r * 0.1) * 20;
      int x = centerX + cos(angle) * (r + waveOffset);
      int y = centerY + sin(angle) * (r + waveOffset);
      
      uint8_t intensity = 255 - (r * 3);
      uint16_t color = gfx->color565(intensity, intensity/2, intensity/3);
      if (x >= 0 && x < getAnimationWidth() && y >= 0 && y < getAnimationHeight()) {
        gfx->drawPixel(x, y, color);
      }
    }
  }
...
void drawApollonian() {
  ...
  for (int i = 0; i < 8; i++) {
    float angle = i * PI / 4 + packTime;
    int radius = 20 + sin(packTime + i) * 15;
    int x = centerX + cos(angle) * (50 + sin(packTime * 2 + i) * 30);
    int y = centerY + sin(angle) * (50 + cos(packTime * 2 + i) * 30);
    
    uint8_t hue = (i * 32 + (int)(packTime * 50)) % 256;
    uint16_t color = gfx->color565(
      (sin(hue * 0.024) + 1) * 127,
      (sin(hue * 0.024 + 2) + 1) * 127,
      (sin(hue * 0.024 + 4) + 1) * 127
    );
...
void drawBarcode() {
  ...
  fillScreenExceptClock(RGB565_WHITE);
  
  int barWidth = 4 + sin(barcodeTime) * 2;
  for (int x = 0; x < getAnimationWidth(); x += barWidth * 2) {
    if ((x / barWidth + (int)barcodeTime) % 2 == 0) {
      gfx->fillRect(x, 0, barWidth, getAnimationHeight(), RGB565_BLACK);
    }
  }
  
  // Add some color variation
  for (int i = 0; i < 10; i++) {
    int x = (i * getAnimationWidth() / 10 + (int)(barcodeTime * 20)) % getAnimationWidth();
    uint16_t color = gfx->color565(255, 0, 128);
    gfx->fillRect(x, 0, 2, getAnimationHeight(), color);
  }
}
...
void drawBlaster() {
  // Space shooter simulation - fast particles
  static float shooterTime = 0;
  shooterTime += 0.1f;
  ...
  for (int i = 0; i < 30; i++) {
    int x = (i * 37 + (int)(shooterTime * 20)) % getAnimationWidth();
    int y = (i * 23) % getAnimationHeight();
  ...
  for (int i = 0; i < 8; i++) {
    int x = 20 + i * 35;
    int y = ((int)(shooterTime * 100 + i * 50)) % getAnimationHeight();
    uint16_t color = gfx->color565(255, 0, 128);
    gfx->drawLine(x, y, x, y+10, color);
  }
  
  // Draw "ship" at bottom
  int shipX = getAnimationWidth()/2 + sin(shooterTime) * 50;
  gfx->fillTriangle(shipX-5, getAnimationHeight()-10, shipX+5, getAnimationHeight()-10, 
                    shipX, getAnimationHeight()-20, RGB565_CYAN);
}

void drawBlitSpin() {
  // Spinning bitmap effect - geometric shapes
  ...
  for (int i = 0; i < 6; i++) {
    float angle = spinTime + i * PI / 3;
    int size = 15 + i * 5;
    int x1 = centerX + cos(angle) * size - size/2;
    int y1 = centerY + sin(angle) * size - size/2;
    
    uint8_t hue = (i * 42 + (int)(spinTime * 30)) % 256;
    uint16_t color = gfx->color565(
      (sin(hue * 0.024) + 1) * 127,
      (sin(hue * 0.024 + 2) + 1) * 127,
      (sin(hue * 0.024 + 4) + 1) * 127
    );
    gfx->drawRect(x1, y1, size, size, color);
  }
}

void drawBouboule() {
  // Particle effects - bouncing balls
  static float particleTime = 0;
  particleTime += 0.03f;
  
  fillScreenExceptClock(RGB565_BLACK);
  
  // Generate dynamic bouncing particles
  for (int i = 0; i < 12; i++) {
    float x = getAnimationWidth()/2 + sin(particleTime + i) * 100;
    float y = getAnimationHeight()/2 + cos(particleTime + i * 1.3) * 80;
    
    // Bounce off edges
    if (x < 0 || x >= getAnimationWidth()) x = getAnimationWidth()/2;
    if (y < 0 || y >= getAnimationHeight()) y = getAnimationHeight()/2;
    
    uint16_t color = gfx->color565(
      255 - i * 20,
      128 + sin(particleTime + i) * 127,
      i * 20
    );
    gfx->fillCircle((int)x, (int)y, 4, color);
  }
}

void drawBoxFit() {
  // Box fitting algorithm - tessellation
  static float fitTime = 0;
  fitTime += 0.02f;
  
  fillScreenExceptClock(RGB565_BLACK);
  
  int boxSize = 15 + sin(fitTime) * 8;
  for (int y = 0; y < getAnimationHeight(); y += boxSize + 2) {
    for (int x = 0; x < getAnimationWidth(); x += boxSize + 2) {
      float phase = (x + y) * 0.02f + fitTime;
      if (sin(phase) > 0) {
        uint8_t intensity = (sin(phase) + 1) * 127;
        uint16_t color = gfx->color565(intensity, intensity/2, 255 - intensity);
        gfx->fillRect(x, y, boxSize, boxSize, color);
      }
    }
  }
}
...
void drawBraid() {
  // Mathematical braids - interwoven curves
  static float braidTime = 0;
  braidTime += 0.04f;
  
  fillScreenExceptClock(RGB565_BLACK);
  
  // Draw 3 interwoven sine waves
  for (int strand = 0; strand < 3; strand++) {
    uint16_t color = (strand == 0) ? RGB565_RED : (strand == 1) ? RGB565_GREEN : RGB565_BLUE;
    
    for (int x = 0; x < getAnimationWidth() - 1; x += 2) {
      float wave1 = sin((x * 0.02f) + braidTime + strand * 2.1f) * 40;
      float wave2 = sin((x * 0.015f) + braidTime * 1.3f + strand * 2.1f) * 20;
      int y = getAnimationHeight()/2 + wave1 + wave2;
      
      if (y >= 0 && y < getAnimationHeight()) {
        gfx->drawPixel(x, y, color);
        gfx->drawPixel(x, y+1, color); // Thicker line
      }
    }
  }
}
...
void drawBubbles() {
  // Bubble simulation - rising circles
  static float bubbleTime = 0;
  bubbleTime += 0.02f;
  ...
  for (int i = 0; i < 15; i++) {
    float x = 30 + (i * 19) % getAnimationWidth();
    int yOffset = ((int)(bubbleTime * 30) + i * 20) % (getAnimationHeight() + 50);
    float y = getAnimationHeight() - yOffset;
    int size = 3 + (i % 5);
    
    if (y >= -size && y < getAnimationHeight()) {
      // Bubble with highlight
      uint16_t bubbleColor = gfx->color565(100, 150, 255);
      gfx->drawCircle((int)x, (int)y, size, bubbleColor);
      gfx->drawPixel((int)x - size/2, (int)y - size/2, RGB565_WHITE); // Highlight
    }
  }
}

void drawBumps() {
  // Bump mapping effect - 3D-looking surface
  static float bumpTime = 0;
  bumpTime += 0.03f;
  
  // Sample every 4 pixels for performance
  for (int y = 0; y < getAnimationHeight(); y += 4) {
    for (int x = 0; x < getAnimationWidth(); x += 4) {
      float height = sin(x * 0.1f + bumpTime) + cos(y * 0.08f + bumpTime * 1.2f);
      height += sin((x + y) * 0.05f + bumpTime * 0.7f) * 0.5f;
      
      uint8_t intensity = ((height + 2.5f) / 5.0f) * 255;
      uint16_t color = gfx->color565(intensity, intensity * 0.7f, intensity * 0.5f);
...
void drawCompass() {
  ...
  // Draw compass circle
  gfx->drawCircle(centerX, centerY, radius, RGB565_WHITE);
  gfx->drawCircle(centerX, centerY, radius - 5, gfx->color565(128, 128, 128));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    gfx->drawLine(x1, y1, x2, y2, RGB565_YELLOW);
  }
  
  // Spinning needle
  float needleAngle = compassTime;
  int needleX = centerX + cos(needleAngle) * (radius - 15);
  int needleY = centerY + sin(needleAngle) * (radius - 15);
  gfx->drawLine(centerX, centerY, needleX, needleY, RGB565_RED);
  gfx->fillCircle(centerX, centerY, 3, RGB565_RED);
}

void drawCoral() {
  // Coral growth patterns - branching structures
  static float coralTime = 0;
  coralTime += 0.02f;
  
  fillScreenExceptClock(RGB565_BLACK);
  
  // Generate coral branches from bottom
  for (int branch = 0; branch < 8; branch++) {
    int baseX = 30 + branch * 35;
    int baseY = getAnimationHeight() - 10;
    
    // Draw branching coral
    for (int segment = 0; segment < 15; segment++) {
      float growth = coralTime + branch * 0.5f + segment * 0.2f;
      int x = baseX + sin(growth) * segment * 2;
      int y = baseY - segment * 8 - sin(growth * 2) * 5;
      
      if (y >= 0 && y < getAnimationHeight() && x >= 0 && x < getAnimationWidth()) {
        uint8_t intensity = 255 - segment * 15;
        uint16_t color = gfx->color565(intensity, intensity/2, intensity/3);
        gfx->fillCircle(x, y, 2 + (segment % 3), color);
      }
    }
  }
}
...
void drawCritical() {
  // Critical phenomena - phase transition visualization
  static float criticalTime = 0;
  criticalTime += 0.04f;
  
  fillScreenExceptClock(RGB565_BLACK);
  
  // Create critical phase transition patterns
  for (int y = 0; y < getAnimationHeight(); y += 6) {
    for (int x = 0; x < getAnimationWidth(); x += 6) {
      float distance = sqrt((x - getAnimationWidth()/2) * (x - getAnimationWidth()/2) + 
                           (y - getAnimationHeight()/2) * (y - getAnimationHeight()/2));
      float phase = sin(distance * 0.05f + criticalTime) * cos(criticalTime + distance * 0.02f);
      
      if (phase > sin(criticalTime * 0.7f)) { // Critical threshold
        uint8_t intensity = (phase + 1) * 127;
        uint16_t color = gfx->color565(255, intensity, 0);
        gfx->fillRect(x, y, 4, 4, color);
      }
    }
  }
...
void drawCrystal() {
  ...
  for (int layer = 0; layer < 6; layer++) {
    int sides = 6; // Hexagonal crystals
    int radius = 15 + layer * 12 + sin(crystalTime + layer) * 5;
    
    for (int i = 0; i < sides; i++) {
      float angle1 = (i * 2.0f * PI / sides) + crystalTime * 0.1f;
      float angle2 = ((i + 1) * 2.0f * PI / sides) + crystalTime * 0.1f;
...
void drawCWaves() {
  // Colored wave interference - enhanced wave patterns
  static float waveTime = 0;
  waveTime += 0.05f;
  
  // Sample every 3 pixels for performance
  for (int y = 0; y < getAnimationHeight(); y += 3) {
    for (int x = 0; x < getAnimationWidth(); x += 3) {
      // Multiple wave sources with different frequencies
      float wave1 = sin(x * 0.05f + waveTime);
      float wave2 = sin(y * 0.06f + waveTime * 1.3f);
      float wave3 = sin((x + y) * 0.03f + waveTime * 0.7f);
      float combined = (wave1 + wave2 + wave3) / 3.0f;
      
      uint8_t r = (sin(combined + waveTime) + 1) * 127;
      uint8_t g = (sin(combined + waveTime + 2) + 1) * 127;
      uint8_t b = (sin(combined + waveTime + 4) + 1) * 127;
...
void drawCynosure() {
  // Cynosure pattern - center of attention focal point
  static float cynosureTime = 0;
  cynosureTime += 0.04f;
  ...
  // Draw radiating lines toward center
  for (int angle = 0; angle < 360; angle += 15) {
    float a = angle * PI / 180 + cynosureTime * 0.2f;
    float pulse = sin(cynosureTime * 2 + angle * 0.1f);
    int length = 80 + pulse * 20;
    
    int x1 = centerX + cos(a) * length;
    int y1 = centerY + sin(a) * length;
    
    if (x1 >= 0 && x1 < getAnimationWidth() && y1 >= 0 && y1 < getAnimationHeight()) {
      uint8_t intensity = (pulse + 1) * 127;
      uint16_t color = gfx->color565(intensity, 255, intensity);
      gfx->drawLine(centerX, centerY, x1, y1, color);
    }
  }
  
  // Pulsing center
  int centerRadius = 8 + sin(cynosureTime * 3) * 4;
  gfx->fillCircle(centerX, centerY, centerRadius, RGB565_WHITE);
}


void drawDeco() {
  // Art deco patterns - geometric elegance
  static float decoTime = 0;
  decoTime += 0.02f;
  ...
  int centerX = getAnimationWidth() / 2;
  int centerY = getAnimationHeight() - 10;
  
  for (int fan = 0; fan < 3; fan++) {
    int fanCenterX = centerX + (fan - 1) * 80;
    
    for (int ray = 0; ray < 12; ray++) {
      float angle = (ray * PI / 6) + decoTime + fan * 0.5f;
      int length = 60 + sin(decoTime + ray + fan) * 20;
      
      int x = fanCenterX + cos(angle) * length;
      int y = centerY - sin(angle) * length;
      
      if (x >= 0 && x < getAnimationWidth() && y >= 0 && y < getAnimationHeight()) {
        uint16_t color = (ray % 2 == 0) ? RGB565_YELLOW : RGB565_MAGENTA;
        gfx->drawLine(fanCenterX, centerY, x, y, color);
      }
    }
  }
}
