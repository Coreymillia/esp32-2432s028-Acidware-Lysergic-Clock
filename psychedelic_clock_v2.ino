...
void drawClock() {
  ...
  if (rtcAvailable) {
    ...
    sprintf(dateStr, "Internal Clock");
  }
  
  // Draw semi-transparent background for clock
  int clockWidth = 140;
  int clockHeight = 50;
  int clockX = (gfx->width() - clockWidth) / 2;
  int clockY = gfx->height() - clockHeight - 10;
  ...
  gfx->setTextColor(RGB565_WHITE);
  gfx->setTextSize(2);
  gfx->setCursor(clockX + 5, clockY + 5);
  gfx->print(timeStr);
  
  // Draw date
  gfx->setTextSize(1);
  gfx->setCursor(clockX + 5, clockY + 30);
  gfx->print(dateStr);
}
