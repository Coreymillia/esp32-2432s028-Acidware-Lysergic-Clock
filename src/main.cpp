...
void setup() {
  ...
  gfx->invertDisplay(true);      // THIS MUST BE TRUE for correct black
  Serial.println("Display inversion enabled - BLACK should now be BLACK!");
...
void drawClock() {
  ...
  int clockWidth = 90;
  int clockHeight = 25;
  int clockX = gfx->width() - clockWidth - 5;
  int clockY = gfx->height() - clockHeight - 5;
  
  // Draw semi-transparent black background for clock
  gfx->fillRect(clockX, clockY, clockWidth, clockHeight, gfx->color565(0, 0, 0));
  
  // Draw time only
  gfx->setTextColor(RGB565_WHITE);
  gfx->setTextSize(1);
  gfx->setCursor(clockX + 3, clockY + 8);
  gfx->print(timeStr);
}

// Helper function to clear screen while preserving clock area
void fillScreenExceptClock(uint16_t color) {
  if (!showClock) {
  ...
  // Clock area coordinates (same as drawClock)
  int clockWidth = 90;
  int clockHeight = 25;
  int clockX = gfx->width() - clockWidth - 5;
  int clockY = gfx->height() - clockHeight - 5;
  
  // Fill screen in sections to avoid clock area
  // Top section (full width, up to clock)
  if (clockY > 0) {
    gfx->fillRect(0, 0, gfx->width(), clockY, color);
  }
  
  // Bottom section (full width, below clock)
  int bottomY = clockY + clockHeight;
  if (bottomY < gfx->height()) {
    gfx->fillRect(0, bottomY, gfx->width(), gfx->height() - bottomY, color);
  }
  
  // Left section (beside clock)
  if (clockX > 0) {
    gfx->fillRect(0, clockY, clockX, clockHeight, color);
  }
}
...
void drawMatrixRain() {
  ...
  for (int i = 0; i < MAX_DROPS; i++) {
    ...
    if (drops[i].active) {
      ...
      if (drops[i].y > gfx->height() + drops[i].length * 8) {
...
void drawDNAHelix() {
  ...
  // Draw DNA strands
  for (float y = 0; y < gfx->height() - 40; y += 3) {
...
void drawLightning() {
  ...
  if (striking) {
    ...
    for (int bolt = 0; bolt < 2; bolt++) {
      ...
      for (int segment = 0; segment < 15 && y < gfx->height() - 10; segment++) {
...
void drawVoronoiLava() {
  ...
  if (!initialized) {
    for (int i = 0; i < 8; i++) {
      ...
      voronoiPoints[i].y = random(gfx->height());
  ...
  if (random(100) < 15) {
    for (int i = 0; i < 50; i++) {
      if (!crystals[i].active) {
        ...
        crystals[i].y = random(gfx->height());
        crystals[i].size = 1;
